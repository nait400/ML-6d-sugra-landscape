...
Models labeled with cluster_id = -1 are treated are outliers and do not fall
...
cluster_select_eps = 0.025 #prev: 0.037, 0.025
num_samples = 5*10**5
save_data = True
...
cor_dir = 'clusters/cluster-data/gram-ae/v-02-02/'
...
for fname in files:
    print(fname)
    all_coords.append(np.loadtxt(fname, delimiter=","))
    partitions.append(len(np.vstack(all_coords)))
...
print('Splitting data into ' + str(len(files)) + 
